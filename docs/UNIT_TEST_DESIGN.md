...
> **📅 最后更新**: 2026-05-13  
> **🔄 重大变更**: 集成测试已从 ohosTest 迁移到 main 模块，提供可视化验证和快速迭代能力。详见 [4.2 Main 模块集成测试](#42-main-模块集成测试-新增)。
...
- [4. 逐步集成测试模块设计](#4-逐步集成测试模块设计)
  - [4.1 集成测试策略](#41-集成测试策略)
    - [4.1.1 HarmonyOS 官方测试分层架构](#411-harmonyos-官方测试分层架构)
    - [4.1.2 集成测试原则（HarmonyOS 官方推荐）](#412-集成测试原则harmonyos-官方推荐)
    - [4.1.3 测试文件组织（HarmonyOS 官方规范）](#413-测试文件组织harmonyos-官方规范)
    - [4.1.4 Mock 测试范式（HarmonyOS 官方推荐）](#414-mock-测试范式harmonyos-官方推荐)
  - [4.2 Main 模块集成测试（⭐ 新增）](#42-main-模块集成测试-新增)
    - [4.2.1 架构设计](#421-架构设计)
    - [4.2.2 核心组件](#422-核心组件)
    - [4.2.3 使用方法](#423-使用方法)
    - [4.2.4 扩展示例](#424-扩展示例)
  - [4.3 NAPI 桥接层集成测试](#43-napi-桥接层集成测试)
  - [4.3 渲染管线集成测试](#43-渲染管线集成测试)
  - [4.4 Surface 管理集成测试（HarmonyOS 官方范式）](#44-surface-管理集成测试harmonyos-官方范式)
    - [4.4.1 Surface 生命周期测试（离屏模式 - 单元测试）](#441-surface-生命周期测试离屏模式---单元测试)
    - [4.4.2 Window Surface 真实测试（⭐ 重点）](#442-window-surface-真实测试-重点)
  - [4.5 端到端集成测试](#45-端到端集成测试)
  - [4.6 压力测试（HarmonyOS 官方推荐）](#46-压力测试harmonyos-官方推荐)
    - [4.6.1 长时间运行测试](#461-长时间运行测试)
    - [4.6.2 页面切换压力测试](#462-页面切换压力测试)
...
- [7. 测试最佳实践](#7-测试最佳实践)
  - [7.1 HarmonyOS 官方测试原则](#71-harmonyos-官方测试原则)
  - [7.2 测试用例设计规范](#72-测试用例设计规范)
  - [7.3 Mock 使用指南](#73-mock-使用指南)
  - [7.4 测试覆盖率要求](#74-测试覆盖率要求)
  - [7.5 CI/CD 集成建议](#75-cicd-集成建议)
- [8. Instrument Test 运行指南](#8-instrument-test-运行指南)
  - [8.1 环境准备](#81-环境准备)
  - [8.2 构建测试包](#82-构建测试包)
  - [8.3 安装和运行](#83-安装和运行)
  - [8.4 查看测试结果](#84-查看测试结果)
  - [8.5 Instrument Test vs Local Test](#85-instrument-test-vs-local-test)
  - [8.6 CI/CD 集成示例](#86-cicd-集成示例)
- [9. 常见问题与解决方案](#9-常见问题与解决方案)
  ...
  - [9.5 设备未连接](#95-设备未连接)
  - [9.6 为什么单元测试没有暴露 Surface 问题？](#96-为什么单元测试没有暴露-surface-问题)
...
#### 4.1.1 HarmonyOS 官方测试分层架构

根据 HarmonyOS 官方文档和最佳实践，测试应分为以下四层：

```
┌─────────────────────────────────────────┐
│ Level 4: UI 自动化测试 (UiTest)          │  ← 模拟用户交互、页面切换
│ - 测试完整的页面生命周期                  │
│ - 测试 Surface 创建/销毁流程              │
│ - 测试错误恢复机制                        │
│ - 使用 @ohos.UiTest 框架                 │
...
│ - 长时间运行压力测试                      │
├─────────────────────────────────────────┤
│ Level 2: 集成测试（main 模块）⭐ 新增     │  ← 本章重点
│ - 在真实应用环境中测试组件                │
│ - 可视化验证渲染效果                      │
│ - 快速迭代和调试                          │
│ - 无需 Hypium/UiTest 框架                │
│ - 位置：entry/src/main/ets/integration/  │
├─────────────────────────────────────────┤
│ Level 1: 单元测试（ohosTest）             │  ← 已完成
│ - C++ 层单个类测试（离屏 Pbuffer 模式）   │
│ - ArkTS 层单个组件测试                   │
│ - Instrument Test 模式                   │
│ - 使用 Mock 隔离系统依赖                  │
...
**关键区别**：
- **Level 1 单元测试**：使用离屏 Pbuffer，快速验证核心逻辑，无需真实 Surface
- **Level 2 集成测试**：在 main 模块中直接测试，使用真实 XComponent Surface，可视化验证
- **Level 3 系统测试**：端到端完整流程，真机/模拟器运行
- **Level 4 UI 自动化**：模拟用户操作，验证页面切换和后台化场景

#### 4.1.2 集成测试原则（HarmonyOS 官方推荐）

1. **自底向上**: 从底层模块开始，逐步向上集成
2. **隔离依赖**: 使用 MockKit 或 import mock 隔离外部依赖
3. **真实环境**: 在接近生产环境的条件下测试（使用真实 Window Surface）
4. **渐进式**: 每次只集成为一个新模块
5. **可重复**: 测试结果稳定可靠
6. **双模式测试**: 
   - 离屏模式（Pbuffer）：用于快速单元测试
   - 窗口模式（Window Surface）：用于真实集成测试
7. **生命周期覆盖**: 必须测试 Surface 创建、销毁、页面切换等场景

#### 4.1.3 测试文件组织（HarmonyOS 官方规范）

**主代码结构（entry/src/main）**：

```
entry/src/main/ets/
├── components/rendering/
│   └── ArkZeroRenderer.ets             # ArkTS 渲染组件
├── entryability/
│   └── EntryAbility.ets                # 应用入口
├── pages/
│   ├── Index.ets                       # 首页
│   └── SurfaceDemoPage.ets             # Surface 演示页
└── integration/                        # ⭐ Level 2: 集成测试（main 模块）
    ├── components/
...
**Instrument Test 结构（entry/src/ohosTest）**：

```
entry/src/ohosTest/ets/                  # Instrument Test 目录（真机/模拟器运行）
├── test/
│   ├── List.test.ets                    # 测试入口文件
│   ├── native/                          # Native 层单元测试（离屏模式）
│   │   ├── PerformanceMonitor.test.ets
│   │   ├── PixelFormatConverter.test.ets
│   │   ├── RenderQueue.test.ets
│   │   ├── TextureManager.test.ets
│   │   ├── EGLContextManager.test.ets
│   │   ├── YUVShaderManager.test.ets
│   │   ├── GLESBackend.test.ets         # ⭐ 使用 InitializeOffscreen
...
**Local Test 结构（entry/src/test）**：

```
entry/src/test/                          # Local Test 目录（本地 JVM 运行）
└── List.test.ets                        # 纯 ETS/JS 逻辑测试（无 Native 依赖）
```
└── window_manager.mock.ets              # WindowManager Mock
```

---

### 4.1.4 Mock 测试范式（HarmonyOS 官方推荐）

当被测代码依赖系统服务（如 Surface、EGL、NativeWindow）时，HarmonyOS 官方提供两种 Mock 能力：

#### **方式 A：MockKit 方法打桩**

适用于 Instrument Test 和 Local Test，通过 Hypium/hamock 的 MockKit 给方法打桩。

**示例代码**：

```typescript
import { describe, it, expect, MockKit, when } from '@ohos/hypium';
import { SurfaceManager } from '../../../../../main/ets/components/rendering/SurfaceManager';

describe('SurfaceLifecycle.MockTests', () => {
  it('MOCK-001: Should handle surface destruction gracefully', 0, async () => {
    const mocker = new MockKit();
    
    // ⭐ Mock NativeWindow 创建
    const mockCreateWindow = mocker.mockFunc(
      SurfaceManager,
      'CreateNativeWindow'  // 方法名
    );
    
    // 模拟返回一个有效的 NativeWindow 指针
    const mockNativeWindowPtr = 0x12345678;
    when(mockCreateWindow)('test_surface_id').thenReturn(mockNativeWindowPtr);
    
    // ⭐ Mock Surface 销毁事件
    const mockDestroy = mocker.mockFunc(
      EGLContextManager,
      'Destroy'
    );
    when(mockDestroy)().thenReturn(true);
    
    // 执行测试
    const renderer = new ArkZeroRenderer(config);
    await renderer.initialize('test_surface_id');
    expect(renderer.isInitialized()).assertTrue();
    
    // 模拟 Surface 销毁
    await simulateSurfaceDestruction(renderer);
    
    // 验证资源已清理
    expect(renderer.isInitialized()).assertFalse();
...
#### **方式 B：import mock 模块替换**

适用于 Stage 工程 API 11+，通过 `mock-config.json5` 将某个 module 映射到 Mock 实现。

**步骤 1：创建 Mock 配置文件**

文件位置：`entry/src/mock/mock-config.json5`

```json5
{
  "@ohos.window": {
    "source": "src/mock/window_manager.mock.ets"
  },
  "libnativerender.so": {
    "source": "src/mock/nativerender.mock.ets"
  }
}
```
...
文件位置：`entry/src/mock/nativerender.mock.ets`

```typescript
/**
 * Native Render 模块的 Mock 实现
 * 用于隔离真实的 Native 代码，快速测试 ArkTS 层逻辑
 */
export default {
  /**
   * Mock create 方法
   */
  create: async (surfaceId: string, width: number, height: number, format: number): Promise<number> => {
    console.log(`[MOCK] Creating renderer with surfaceId: ${surfaceId}, size: ${width}x${height}`);
   ...
   * 可以模拟各种场景：成功、失败、延迟等
   */
  renderFrame: async (handle: number, pixelData: ArrayBuffer, width: number, height: number): Promise<void> => {
    console.log(`[MOCK] Rendering frame: ${width}x${height}`);
    
    // 模拟随机失败（10% 概率），用于测试错误处理
    if (Math.random() < 0.1) {
      throw new Error('[MOCK] Simulated EGL_BAD_SURFACE error');
    }
   ...
   * Mock resize 方法
   */
  resize: async (handle: number, width: number, height: number): Promise<void> => {
    console.log(`[MOCK] Resizing to: ${width}x${height}`);
   ...
   * Mock destroy 方法
   */
  destroy: async (handle: number): Promise<void> => {
    console.log(`[MOCK] Destroying renderer: ${handle}`);
  },
  
  /**
   * Mock getPerformanceStats 方法
   */
...
1. ✅ **快速执行**：无需真机/模拟器，本地即可运行
2. ✅ **可重复性**：Mock 行为可控，测试结果稳定
3. ✅ **隔离依赖**：不依赖真实的系统服务和硬件
4. ✅ **错误注入**：可以轻松模拟各种异常场景
5. ✅ **并行测试**：多个测试用例可以同时运行，互不干扰

**注意事项**：

- ❌ Mock 测试不能替代集成测试，必须配合真实环境测试
- ✅ Mock 主要用于单元测试和快速迭代开发
- ✅ 集成测试和 UI 自动化测试必须使用真实 Window Surface

---

### 4.2 Main 模块集成测试（⭐ 新增）

**设计理念**: 在 main 模块中直接进行集成测试，无需依赖 Hypium 或 UiTest 框架，提供可视化验证和快速迭代能力。

#### 4.2.1 架构设计

```
┌─────────────────────────────────────────┐
│  IntegrationTestPage (UI 层)            │
│  - XComponent Surface                   │
│  - 控制面板                              │
│  - 测试结果面板                          │
└──────────────┬──────────────────────────┘
...
└──────────────┬──────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────┐
│  ArkZeroRenderer (被测试组件)            │
│  - initialize(surfaceId)                │
│  - renderFrame(buffer, w, h)            │
│  - resize(w, h)                         │
│  - dispose()                            │
└──────────────┬──────────────────────────┘
               │
...
**1. TestResultPanel.ets** - 测试结果面板组件
...
export struct TestResultPanel {
  ...
  build() {
    Column() {
      ...
      Row() {
        Text('🧪 集成测试结果')
        Text(`${this.passRate} (${this.passCount}/${this.completedTests})`)
      }
      
      // 进度条
      Progress({ value: this.completedTests, total: this.totalTests })
...
**2. RendererIntegration.test.ets** - 渲染器集成测试类

包含 5 个核心测试用例：

| 测试 ID | 名称 | 目标 | 预计耗时 |
|---------|------|------|----------|
| RI-001 | 初始化渲染器 | 验证渲染器创建和 Surface 绑定 | ~2秒 |
| RI-002 | 渲染单帧 | 测试基本渲染功能 | ~1秒 |
| RI-003 | 调整渲染尺寸 | 验证动态 resize 功能 | ~2秒 |
| RI-004 | 连续渲染多帧 | 测试多帧渲染性能和稳定性 | ~5秒 |
| RI-005 | 资源清理 | 验证 dispose 正确释放资源 | ~1秒 |
...
**3. IntegrationTestPage.ets** - 集成测试主页面
...
struct IntegrationTestPage {
  ...
  private async startIntegrationTests(): Promise<void> {
    ...
    const test = new RendererIntegrationTest(this.surfaceId, (results) => {
  ...
  build() {
    Stack() {
      ...
      Column() {
        Button('开始集成测试')
          .onClick(() => this.startIntegrationTests())
      }
...
1. 在首页点击 **"🧪 运行集成测试"** 按钮
2. 等待 XComponent Surface 初始化（约 1-2 秒）
3. 看到 "✅ Surface 已就绪" 后，点击 **"开始集成测试"** 按钮
4. 观察测试结果面板
...
步骤 1: 在 `tests/` 目录下创建新的测试文件

```typescript
// EGLContextLifecycle.test.ets
...
步骤 2: 在 `IntegrationTestPage.ets` 中调用
...
1. **零配置** - 不需要配置 Hypium 或 UiTest 框架
2. **所见即所得** - 直接在屏幕上看到渲染效果
3. **快速反馈** - 修改代码后立即可测试，无需重新构建测试包
4. **易于调试** - 可以使用 DevEco Studio 的完整调试功能
5. **可视化强** - 实时进度条、颜色编码、统计信息一目了然

**与 ohosTest 对比**

| 特性 | main 模块集成测试 | ohosTest Instrument Test |
|------|------------------|-------------------------|
| **运行环境** | 真实应用环境 | 独立测试环境 |
| **启动速度** | ⚡ 快速（秒级） | 🐌 较慢（需要安装测试包） |
| **调试便利性** | ✅ 直接查看 UI | ❌ 需要通过日志 |
| **可视化验证** | ✅ 直观看到渲染效果 | ❌ 只能看日志 |
| **依赖框架** | ❌ 无需 Hypium/UiTest | ✅ 需要测试框架 |
| **迭代速度** | ⚡ 修改后立即生效 | 🐌 需要重新构建测试包 |
| **适用场景** | 功能验证、性能测试 | 自动化回归测试 |
...
**关键优势**：
1. **零配置** - 不需要配置测试框架
2. **所见即所得** - 直接在屏幕上看到渲染结果
3. **快速反馈** - 修改代码后立即可测试
4. **易于调试** - 可以使用 DevEco Studio 的调试功能
...
**注意**：这些测试使用离屏 Pbuffer 模式，无法发现真实的 Window Surface 问题。

---

#### 4.4.2 Window Surface 真实测试（⭐ 重点）

**测试文件**: `entry/src/ohosTest/ets/integration/surface/WindowSurface.test.ets`

**测试目标**: 使用真实的 NativeWindow 测试 Surface 生命周期，发现 EGL_BAD_SURFACE 等问题

| 测试用例ID | 测试场景 | 操作步骤 | 预期结果 | 优先级 |
|-----------|---------|---------|---------|--------|
| WS-REAL-001 | 真实 Surface 创建 | 通过 XComponent 获取 surfaceId | NativeWindow 创建成功 | P0 |
| WS-REAL-002 | 渲染多帧 | 连续渲染 100 帧 | 所有帧成功，无内存泄漏 | P0 |
| WS-REAL-003 | Surface 失效处理 | 导航到其他页面触发销毁 | 优雅处理，不崩溃 | P0 |
| WS-REAL-004 | EGL_BAD_SURFACE 恢复 | 模拟 Surface 失效后继续渲染 | 检测到错误，停止渲染 | P0 |
| WS-REAL-005 | 页面切换测试 | 切换到其他页面再返回 | Surface 重新创建成功 | P0 |
| WS-REAL-006 | 后台化测试 | 按 Home 键后台化 10 秒 | 回到前台后渲染恢复 | P1 |
| WS-REAL-007 | VSync 超时处理 | 长时间无 VSync 信号 | 检测超时，记录日志 | P1 |
...
export default function WindowSurfaceTest() {
  describe('WindowSurface.RealTests', () => {
    
    it('WS-REAL-001: Should create real NativeWindow from XComponent', 0, async () => {
      const driver = await Driver.create();
      
      // 1. 启动包含 XComponent 的页面
      await driver.startAbility('com.sample.component/SurfaceDemoPage');
      await sleep(2000);  // 等待 XComponent 加载
      
      // 2. 验证 Surface 已创建
      const xComponent = await driver.findComponent(
        On.id('surface_renderer').build()
      );
      await xComponent.waitForExist(5000);
      
      // 3. 检查渲染是否正常
      const frameCount1 = await getFrameCount(driver);
      await sleep(1000);
      const frameCount2 = await getFrameCount(driver);
      
      expect(frameCount2 > frameCount1).assertTrue();
      
      await driver.terminate();
    });

    it('WS-REAL-003: Should handle surface destruction gracefully', 0, async () => {
      const driver = await Driver.create();
      await driver.startAbility('com.sample.component/SurfaceDemoPage');
      await sleep(2000);
      ...
      // 2. ⭐ 导航到其他页面（触发 Surface 销毁）
      await driver.pressBack();
      await sleep(1000);
      
      // 3. 检查日志，不应有连续的 EGL_BAD_SURFACE 错误
      const logs = await getLogs(driver, 'EGLContextManager');
      const errorCount = logs.filter(log => 
        log.includes('EGL_BAD_SURFACE') || log.includes('Failed to swap buffers')
      ).length;
      
      // 允许少量错误，但不能连续失败
      expect(errorCount).assertLess(5);
      
      await driver.terminate();
    });

    it('WS-REAL-005: Should recover after page navigation', 0, async () => {
      const driver = await Driver.create();
      ...
      // 4. ⭐ 返回原页面（触发 Surface 重新创建）
      await driver.startAbility('com.sample.component/SurfaceDemoPage');
      await sleep(2000);
      
      // 5. 验证重新初始化成功
      const frameCount3 = await getFrameCount(driver);
      await sleep(1000);
      const frameCount4 = await getFrameCount(driver);
      expect(frameCount4 > frameCount3).assertTrue();
      
      await driver.terminate();
    });

    it('WS-REAL-006: Should handle background/foreground transition', 0, async () => {
      const driver = await Driver.create();
      await driver.startAbility('com.sample.component/SurfaceDemoPage');
      await sleep(2000);
      ...
      await sleep(10000);  // ⭐ 等待 10 秒，可能触发 VSync 超时
      
      // 3. 回到前台
      await driver.startAbility('com.sample.component');
      await sleep(2000);
      
      // 4. 验证渲染恢复
      const isRendering = await checkRenderingStatus(driver);
      expect(isRendering).assertTrue();
...
1. ✅ **使用真实的 XComponent Surface**：通过 `Driver.startAbility()` 启动真实页面
2. ✅ **测试页面切换**：使用 `driver.pressBack()` 和 `driver.startAbility()` 模拟导航
3. ✅ **测试后台化**：使用 `driver.pressHome()` 模拟用户按 Home 键
4. ✅ **检查日志**：通过 `getLogs()` 验证没有连续的 EGL 错误
5. ✅ **验证恢复能力**：确保 Surface 销毁后能重新创建
...
**版本历史：**
- v2.0.0 (2026-05-13): 根据 HarmonyOS 官方测试范式重构，新增 Mock 测试、Window Surface 真实测试、压力测试章节
- v1.0.0 (2026-05-12): 初始版本，完成基础单元测试和集成测试设计

---

## 7. 测试最佳实践

### 7.1 HarmonyOS 官方测试原则

根据 HarmonyOS 官方文档，测试应遵循以下核心原则：
...
Level 1: 单元测试（快速、隔离）
  ├─ 目标：验证单个类/方法的正确性
  ├─ 工具：Hypium + MockKit
  ├─ 环境：Local Test 或 Instrument Test
  └─ 特点：离屏 Pbuffer 模式，无需真实 Surface

Level 2: 集成测试（真实环境）
  ├─ 目标：验证模块间协作
  ├─ 工具：Hypium + 真实 NativeWindow
  ├─ 环境：Instrument Test（真机/模拟器）
  └─ 特点：使用 Window Surface，测试生命周期

Level 3: UI 自动化测试（用户场景）
  ├─ 目标：验证用户交互流程
  ├─ 工具：@ohos.UiTest
  ├─ 环境：Instrument Test（真机/模拟器）
  └─ 特点：模拟页面切换、后台化等操作
...
Level 4: 压力测试（稳定性）
  ├─ 目标：检测内存泄漏、资源累积
  ├─ 工具：@ohos.UiTest + 长时间运行
  ├─ 环境：Instrument Test（真机优先）
  └─ 特点：10-30 分钟连续运行
```
...
- ✅ 用于单元测试，快速验证核心逻辑
- ✅ 无需真实 Surface，执行速度快
- ❌ 无法发现 Window Surface 的生命周期问题

**窗口模式（Window Surface）**：
- ✅ 用于集成测试，验证实战场景
- ✅ 能发现 EGL_BAD_SURFACE 等真实问题
- ❌ 需要真机/模拟器，执行速度慢
...
当被测代码依赖系统服务时，优先使用 Mock：

```typescript
// ✅ 推荐：使用 Mock 隔离依赖
const mocker = new MockKit();
const mockFunc = mocker.mockFunc(Service, 'method');
when(mockFunc)(args).thenReturn(mockValue);
...
```typescript
// ✅ 推荐：TEST-ID + 描述
it('PM-BFN-001: Should begin frame and record timestamp', 0, () => {
  // ...
});

// ✅ 也可接受：只有描述
it('Should begin frame and record timestamp', 0, () => {
  // ...
});
```
...
```typescript
it('TEST-ID: Description', 0, async () => {
  // Arrange（准备）
  const renderer = new ArkZeroRenderer(config);
  await renderer.initialize('surface_id');
...
| 优先级 | 标识 | 比例 | 说明 |
|--------|------|------|------|
| P0 | 必须通过 | 60% | 核心功能，阻塞发布 |
| P1 | 应该通过 | 30% | 重要功能，影响体验 |
| P2 | 可以通过 | 10% | 边缘场景，可选 |
...
**应该使用 Mock**：
- ✅ 依赖系统服务（WindowManager、Surface）
- ✅ 依赖硬件资源（GPU、Camera）
- ✅ 依赖网络请求（HTTP、WebSocket）
- ✅ 依赖文件系统（读写操作）
- ✅ 需要模拟异常场景（超时、错误）
...
```typescript
// ✅ 推荐：Mock 后记得清理
it('Should use mock correctly', 0, async () => {
  const mocker = new MockKit();
  const mockFunc = mocker.mockFunc(Service, 'method');
  when(mockFunc)().thenReturn(true);
...
// ❌ 避免：忘记清理 Mock
it('Should not forget cleanup', 0, async () => {
  const mocker = new MockKit();
  const mockFunc = mocker.mockFunc(Service, 'method');
  when(mockFunc)().thenReturn(true);
...
|---------|---------|-----------|-----------|
| C++ Core | ≥ 80% | ≥ 70% | ≥ 90% |
| NAPI Bridge | ≥ 90% | ≥ 80% | ≥ 95% |
| ArkTS Layer | ≥ 85% | ≥ 75% | ≥ 90% |

#### 7.4.2 关键模块必须 100% 覆盖

- ✅ PerformanceMonitor（性能监控）
- ✅ RenderQueue（渲染队列）
- ✅ EGLContextManager（EGL 上下文管理）
- ✅ TextureManager（纹理管理）
- ✅ PixelFormatConverter（像素格式转换）
...
# 查看报告
# entry/.test/default/intermediates/ohosTest/coverage_data/
```
...
jobs:
  test:
    ...
    - name: Build test package
      run: hvigorw test -p module=entry
    ...
    - name: Upload test report
      ...
      with:
        name: test-report
        path: entry/build/reports/tests/
```
...
### 9.6 为什么单元测试没有暴露 Surface 问题？

#### 9.6.1 问题背景

在实际运行中，我们发现了 `EGL_BAD_SURFACE` 错误和 VSync 超时问题，但单元测试却没有暴露这些问题。这是为什么？
...
| 特性 | 单元测试环境 | 实际运行环境 |
|------|------------|-------------|
| Surface 类型 | EGL_PBUFFER_BIT（离屏） | EGL_WINDOW_BIT（窗口） |
| Surface 来源 | EGL 自动创建 | XComponent 提供 NativeWindow |
| Surface 生命周期 | 由 EGL 管理，稳定 | 受页面生命周期影响，可能失效 |
| SwapBuffers 行为 | 始终成功（离屏不需要交换） | 可能失败（Surface 被销毁） |
...
it('GB-RND-003: Should render multiple frames', 0, async () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    expect(success).assertTrue();  // ✅ 只验证成功
  }
...
this.animationTimer = setInterval(async () => {
  await this.renderer!.renderFrame(this.pixelBuffer!, 1920, 1080);
  // ❌ 可能被页面切换、后台化等事件中断
}, 16);
```
...
根据 HarmonyOS 官方测试范式，应该采用分层测试策略：

**Level 1: 单元测试（离屏）**
```typescript
// entry/src/ohosTest/ets/test/native/GLESBackend.test.ets
// 目标：验证核心逻辑正确性
it('GB-OFF-001: Should initialize offscreen context', 0, async () => {
  const handle: number = await nativerender.createGLESBackend();
  const success: boolean = await nativerender.glesBackendInitialize(
    handle, 640, 480, 0  // ⭐ 使用 InitializeOffscreen
  );
  expect(success).assertTrue();
});
```

**Level 2: 集成测试（真实 Window Surface）**
```typescript
// entry/src/ohosTest/ets/integration/surface/WindowSurface.test.ets
// 目标：验证实战场景
it('WS-REAL-003: Should handle surface destruction gracefully', 0, async () => {
  const driver = await Driver.create();
  await driver.startAbility('com.sample.component/SurfaceDemoPage');
  ...
  // ⭐ 导航到其他页面（触发 Surface 销毁）
  await driver.pressBack();
  await sleep(1000);
  
  // 检查日志，不应有连续的 EGL_BAD_SURFACE 错误
  const logs = await getLogs(driver, 'EGLContextManager');
  const errorCount = logs.filter(log => 
    log.includes('EGL_BAD_SURFACE')
  ).length;
  expect(errorCount).assertLess(5);
});
```
...
**Level 3: UI 自动化测试**
```typescript
// entry/src/ohosTest/ets/ui/LifecycleSimulation.test.ets
// 目标：模拟用户操作
it('UI-LC-001: Should recover after page navigation', 0, async () => {
  const driver = await Driver.create();
  ...
  // 4. 验证重新初始化成功
  const frameCount = await getFrameCount(driver);
  expect(frameCount).assertLarger(0);
});
```

**Level 4: 压力测试**
```typescript
// entry/src/ohosTest/ets/stress/LongRunningRender.test.ets
// 目标：检测内存泄漏和资源累积
it('STRESS-LR-001: Should handle 10-minute continuous rendering', 0, async () => {
  const driver = await Driver.create();
  await driver.startAbility('com.sample.component/SurfaceDemoPage');
  
  const startTime = Date.now();
  const duration = 10 * 60 * 1000;  // 10 分钟
  
  while (Date.now() - startTime < duration) {
    // 每 30 秒检查一次内存使用
    const memoryUsage = await getMemoryUsage(driver);
    if (memoryUsage > MAX_ALLOWED_MEMORY) {
      throw new Error(`Memory leak detected: ${memoryUsage} MB`);
    }
    await sleep(5000);
  }
});
...
1. ✅ **只使用了离屏模式（Pbuffer）**：无法发现 Window Surface 的生命周期问题
2. ✅ **缺少集成测试**：没有测试真实的页面切换和 Surface 销毁场景
3. ✅ **缺少 UI 自动化测试**：没有模拟用户的真实操作
4. ✅ **缺少压力测试**：没有长时间运行以触发 VSync 超时和资源累积问题
5. ✅ **缺少错误注入测试**：没有模拟 EGL_BAD_SURFACE 等错误场景
...
单元测试（离屏） → 验证核心逻辑正确性
   ↓
集成测试（真机） → 验证 Window Surface 生命周期
   ↓  
UI 自动化测试   → 验证页面切换和用户交互
   ↓
压力测试       → 验证长时间运行的稳定性
```
...
- 📝 **单元测试不能替代集成测试**：每种测试都有其局限性
- 📝 **必须使用真实环境测试**：离屏模式无法发现所有问题
- 📝 **Mock 测试要谨慎使用**：Mock 会隐藏真实的问题
- 📝 **测试覆盖率不等于质量**：覆盖率高不代表没有 bug
- 📝 **分层测试是必须的**：不同层次的测试互补，缺一不可

---

## 10. 更新日志

### 2026-05-13 - 集成测试迁移到 main 模块

**重大变更**：

1. **新增 Level 2 集成测试（main 模块）**
   - 位置：`entry/src/main/ets/integration/`
   - 包含：TestResultPanel、RendererIntegrationTest、IntegrationTestPage
   - 优势：可视化验证、快速迭代、无需测试框架

2. **清理 ohosTest 集成测试**
   - 删除：`entry/src/ohosTest/ets/test/integration/`
   - 删除：`entry/src/ohosTest/ets/integration/`
   - 保留：单元测试（native/ 和 arkts/）

3. **更新测试分层架构**
   - Level 1: 单元测试（ohosTest）- 离屏 Pbuffer
   - Level 2: 集成测试（main 模块）⭐ 新增 - 真实 Surface，可视化
   - Level 3: 系统测试 - 端到端流程
   - Level 4: UI 自动化 - @ohos.UiTest

4. **文档更新**
   - 新增 4.2 章节：Main 模块集成测试详细说明
   - 更新 4.1.1 章节：测试分层架构图
   - 更新 4.1.3 章节：测试文件组织结构
   - 添加使用指南和扩展示例
...
- ✅ 更直观的测试结果展示
- ✅ 更简单的测试配置（零框架依赖）
- ⚠️ ohosTest 中不再包含集成测试

**迁移指南**：
- 如需运行集成测试：构建应用后，在首页点击 "🧪 运行集成测试" 按钮
- 如需运行单元测试：继续使用 `hvigorw test` 命令
- 详细使用方法：参见 [4.2.3 使用方法](#423-使用方法)

**相关文档**：
- [WINDOWSURFACE_TEST_STATUS.md](WINDOWSURFACE_TEST_STATUS.md) - WindowSurface 测试状态说明
- [INTEGRATION_TEST_MIGRATION.md](INTEGRATION_TEST_MIGRATION.md) - 集成测试迁移报告
- [QUICK_START_INTEGRATION_TEST.md](QUICK_START_INTEGRATION_TEST.md) - 5分钟快速上手指南
- [entry/src/main/ets/integration/README.md](entry/src/main/ets/integration/README.md) - 集成测试完整文档
