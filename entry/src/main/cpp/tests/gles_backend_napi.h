/*
 * Copyright (c) 2024 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef GLES_BACKEND_NAPI_H
#define GLES_BACKEND_NAPI_H

#include <napi/native_api.h>

namespace NativeXComponentSample {

/**
 * 创建 GLES 渲染后端
 * 
 * ArkTS调用: createGLESBackend(): number
 * 
 * @param env NAPI环境
 * @param info NAPI回调信息
 * @return number - GLESBackend 句柄
 */
napi_value CreateGLESBackend(napi_env env, napi_callback_info info);

/**
 * 销毁 GLES 渲染后端
 * 
 * ArkTS调用: destroyGLESBackend(handle: number): void
 * 
 * @param env NAPI环境
 * @param info NAPI回调信息
 * @return undefined
 */
napi_value DestroyGLESBackend(napi_env env, napi_callback_info info);

/**
 * 初始化 GLES 后端（离屏模式，用于测试）
 * 
 * ArkTS调用: glesBackendInitialize(handle: number, width: number, height: number, format: number): boolean
 * 
 * @param env NAPI环境
 * @param info NAPI回调信息
 * @return boolean - 是否成功
 */
napi_value GLESBackendInitialize(napi_env env, napi_callback_info info);

/**
 * 渲染帧
 * 
 * ArkTS调用: glesBackendRenderFrame(handle: number, buffer: ArrayBuffer, width: number, height: number): boolean
 * 
 * @param env NAPI环境
 * @param info NAPI回调信息
 * @return boolean - 是否成功
 */
napi_value GLESBackendRenderFrame(napi_env env, napi_callback_info info);

/**
 * 检查是否已初始化
 * 
 * ArkTS调用: isGLESBackendInitialized(handle: number): boolean
 * 
 * @param env NAPI环境
 * @param info NAPI回调信息
 * @return boolean - 是否已初始化
 */
napi_value IsGLESBackendInitialized(napi_env env, napi_callback_info info);

} // namespace NativeXComponentSample

#endif // GLES_BACKEND_NAPI_H
